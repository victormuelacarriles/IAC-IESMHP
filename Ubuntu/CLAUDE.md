...
- **2026-09-25 — `1-SetupLiveCD.sh` parado tras `apt-get install zfsutils-linux` (CEIABD)**: el log acaba en `Procesando disparadores para libc-bin` y no avanza. `ps` muestra `apt-get` en estado **`T`** (detenido por señal de control de terminal, no colgado): con stdin = terminal y stdout = pipe a `tee` (`exec > >(tee ...)`), apt se para al volver a tocar el tty tras dpkg. Apareció cuando apt empezó a *actualizar* zfsutils-linux (5 → 5.1). Desbloqueo en caliente: `kill -CONT <pid>`. **Fix**: `</dev/null` en los `apt-get` del bloque ZFS **y** en la llamada `chroot /mnt "$SCRIPT2" </dev/null | tee ...` (el script 2 heredaba el terminal y se paraba igual en su PASO 9). Blindaje de toda la cadena: `exec </dev/null` en `1-SetupLiveCD.sh` (tras `loadkeys`) y en `3-SetupPrimerInicio.sh`; `</dev/null` en los apt-get de `0b-Github.sh` y al lanzar `0b-Github.sh` desde `iac-iesmhp-run.sh` (`0a-CreaISO.sh`). **Regla**: cualquier script que haga `exec > >(tee ...)` y luego `apt-get` debe tener stdin en `/dev/null`. Pista para casos futuros: si un script "se cuelga" en apt, mirar el `STAT` en `ps` antes de nada.
