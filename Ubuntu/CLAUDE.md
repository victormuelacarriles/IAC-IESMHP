...
- **2026-05-20 — `/home/usuario` queda root:root sin skel tras instalar (CEIABD)**: en el chroot, `zfs create -o canmount=on rpool/home/usuario` creaba el dataset pero **no llegaba a montarlo de forma fiable** (altroot=/mnt + parent `rpool/home` canmount=off). El script no verificaba `mountpoint` → `useradd -m`, `cp -aT /etc/skel` y `chown -R` escribían al **ext4 subyacente**, snapshot `@inicial` se tomaba del dataset ZFS vacío, y al reiniciar `zfs-mount.service` montaba el dataset encima ocultando el contenido bueno. Síntoma: GDM «Sin Carpeta Personal», `mkdir` denegado en `/home/usuario`, dataset con `used` mínimo y solo `verLog.sh` (que escribe `3-SetupPrimerInicio.sh`). **Fix**: tras `zfs create` añadir `zfs mount` explícito + `mountpoint -q /home/usuario || (err; exit 1)` antes de tocar nada; `useradd` sin `-m` y `cp skel + chown + chmod 750` incondicionales. Mismo patrón en el helper `nuevo-alumno.sh`. Versión `2-SetupSOdesdeLiveCD.sh`: `23.3` → `23.4`.
